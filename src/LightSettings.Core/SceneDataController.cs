...
namespace LightSettings.Koikatu {
    internal class SceneDataController : SceneCustomFunctionController {
                ...
                foreach (var lightData in saveData) {
                    ...
                    if (loadedItems.TryGetValue(lightData.ObjectId, out var oci)) {
                        if (oci is OCILight ociLight) {
                            ...
                            SetLoadedData(lightData, ociItem.objectItem.GetComponentsInChildren<Light>(true).ToList(), true);
                        }
                    } else if (lightData.ObjectId == -10) {
                        LightSettings.charaLightData = lightData;
                        LightSettings.charaLightSetCountDown = 5;
                    }
                }
                if (LightSettings.charaLightSetCountDown < 0) {
                    LightSettings.charaLightData = new LightSaveData {
                        ObjectId = chaLightID,
        ...
        protected override void OnSceneSave() {
            ...
            // Add chara light
            saveData.Add(LightSettings.charaLightData);
        ...
        internal static void SetLoadedData(LightSaveData lightData, List<Light> lights, bool setState = false) {
        ...
        private static void AddSaveData(List<LightSaveData> saveData, int key, Light light) {
